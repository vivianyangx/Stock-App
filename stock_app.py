...
.stApp { background: #d9dcc8; color: #11121a; }
...
.block-container {
    ...
    background: #f8f8f5;
    border: 1px solid rgba(17,18,26,.10);
    border-radius: 34px;
    box-shadow: 0 28px 70px rgba(30,32,24,.18);
}
.mobile-nav { display:flex; align-items:center; justify-content:space-between; margin:2px 2px 18px; }
.circle-action { width:38px; height:38px; border-radius:50%; display:grid; place-items:center; background:#151620; color:white; font-size:16px; }
.ticker-lockup { text-align:center; }
.ticker-name { font-size:15px; color:#11121a; font-weight:800; letter-spacing:.3px; }
.ticker-company { font-size:9px; color:#85858e; margin-top:3px; }
.price-panel { background:#ececef; border-radius:18px; padding:15px 16px; margin-bottom:10px; }
.eyebrow { color:#8b8b93; font-size:9px; font-weight:650; text-transform:uppercase; letter-spacing:.8px; }
...
.stat-cell { background:#ececef; border-radius:13px; padding:11px 12px; min-height:58px; }
...
.mobile-card { background:#ececef; border-radius:15px; padding:13px 14px; }
.mobile-card .value { color:#161720; font-size:18px; font-weight:800; margin-top:5px; }
.mobile-card .value.lime { color:#aee900; }
.mobile-card .value.purple { color:#6c00f5; }
.mobile-card .value.red { color:#ef6170; }
.detail-card { background:#ececef; border-radius:16px; padding:6px 14px; margin:8px 0; }
...
.mobile-note { color:#8b8b93; font-size:9px; line-height:1.55; padding:8px 3px; }
.stTabs [data-baseweb="tab-list"] { background:#ececef; padding:4px; border-radius:13px; gap:3px; }
.stTabs [data-baseweb="tab"] { background:transparent; border:0; color:#85858e; border-radius:10px; padding:8px 16px; flex:1; }
.stTabs [aria-selected="true"] { background:#151620 !important; color:white !important; border:0 !important; }
...
div[data-testid="stExpander"] { background:white; border:1px solid #e3e3e7; border-radius:15px; overflow:hidden; }
...
def fetch_nasdaq(ticker: str, period: str) -> pd.DataFrame:
    """Keyless Nasdaq fallback for daily US-listed OHLCV history."""
    ...
    raw.attrs['provider'] = 'Nasdaq fallback'
    return raw
...
with tab_price:
    ...
    st.markdown(f"""
    <div class="eyebrow" style="margin:16px 3px 8px;">REFERENCE MAP · NOT ORDERS</div>
    <div class="plan-strip">
      <div class="mobile-card"><div class="eyebrow">Support zone</div><div class="value">${plan['entry_low']:.2f}–${plan['entry_high']:.2f}</div></div>
      <div class="mobile-card"><div class="eyebrow">Risk threshold</div><div class="value red">${plan['stop']:.2f}</div></div>
      <div class="mobile-card"><div class="eyebrow">Resistance 1</div><div class="value purple">${plan['target_1']:.2f}</div></div>
      <div class="mobile-card"><div class="eyebrow">Resistance 2</div><div class="value lime">${plan['target_2']:.2f}</div></div>
    </div>
    <div class="mobile-note">These levels organize observation and risk review. They are not buy, sell or execution instructions.</div>
    """, unsafe_allow_html=True)
...
**Support zone** is an area to watch for price response. It is not a floor. **Risk threshold** is where the current structure deserves review, not an automatic order. **Resistance 1/2** are nearby observed supply references—not price forecasts.
        """)
